...
class Encoder(nn.Module):
    ...
    def __init__(self, in_channels=3, base_dim=64):
        ...
        self.net = nn.Sequential(
            ...
            HKPFE(base_dim),  # 输出56x56（通道128）
            HKPFE(base_dim * 2),  # 输出28x28（通道256）
            HKPFE(base_dim * 4),  # 输出14x14（通道512）
        )
...
class RegionRecoveryModel(nn.Module):
    ...
    def __init__(self):
        ...
        # Shared-weight encoder for the two input images
        self.encoder = Encoder()

        self.attention = CrossRegionAttention()
        self.decoder = Decoder()

        # Used in the supervised recognition stage
        self.conv = nn.Conv2d(512, 1024, kernel_size=1)
        self.pool = nn.AdaptiveAvgPool2d((1, 1))

    def forward(self,orig_img,mixed_img,is_pretrain=False,return_spatial=False):
        # Shared HKPFE encoder
        orig_feat = self.encoder(orig_img)
        mix_feat = self.encoder(mixed_img)

        orig_feat = self.global_dropout(orig_feat)
        mix_feat = self.global_dropout(mix_feat)

        assert orig_feat.shape[-2:] == mix_feat.shape[-2:], \
            f"Feature-map size mismatch: orig {orig_feat.shape} vs mix {mix_feat.shape}"

        # IRSA
        attn_feat = self.attention(orig_feat, mix_feat)

        # Pre-training: directly reconstruct from IRSA feature
        if is_pretrain:
            return self.decoder(attn_feat)

        # Supervised recognition projection
        # [B, 512, 14, 14] -> [B, 1024, 14, 14]
        attn_feat = self.conv(attn_feat)

        # Preserve spatial information for MAFM
        if return_spatial:
            return attn_feat

        # Keep the original behavior for other usages
